#!/usr/bin/env bash
# Run the agentic-ops starter side-by-side demo.
set -euo pipefail
...
echo
echo "========================================================================"
echo "  agentic-ops demo: provider=${AGNO_MODEL_PROVIDER}"
echo "========================================================================"
...
( cd "${PROJECT_DIR}/agent" && "${PYTHON}" -m run_no_kiff )
( cd "${PROJECT_DIR}/agent" && "${PYTHON}" -m run_with_kiff )
