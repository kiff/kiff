...
type genModel struct {
	...
	// AgentRoles are roles that grant at least one action's required
	// permission — the roles a proposing actor needs. OperatorRoles are the
	// remaining oversight/approval roles, held by the human operator. Splitting
	// them keeps the proposer/approver boundary intact in the generated default.
	AgentRoles    []string
...
func buildModel(d scaffoldDescriptor) (genModel, error) {
	...
	// Classify roles: a role that grants any action's required permission is an
	// "agent" (proposer) role; the rest are oversight/approval roles held by the
	// human operator. This keeps proposer != approver in the generated default
	// instead of granting every role to every actor.
	...
	for _, r := range roleNames {
		...
		if grantsAction {
			m.AgentRoles = append(m.AgentRoles, r)
		} else {
			m.OperatorRoles = append(m.OperatorRoles, r)
		}
	}
...
const domainGoTmpl = `// Generated by 'kiff scaffold' — fill in the executor bodies; the wiring
...
{{end}}{{end}}{{range .AgentRoles}}	policy.AssignRole(AgentActor.ID, "{{.}}")
	policy.AssignRole(SystemActor.ID, "{{.}}")
{{end}}{{range .OperatorRoles}}	policy.AssignRole(OperatorActor.ID, "{{.}}")
{{end}}	return policy
}
...
const domainTestGoTmpl = `// Generated by 'kiff scaffold'.
package domain
