...
func (a *ScriptedAgent) Propose(context.Context, AgentRequest) (AgentProposal, error) {
	if len(a.Proposals) == 0 {
		return AgentProposal{
			...
			ReasoningSummary: "scripted cloud remediation agent has no remaining proposals",
			Confidence:       1,
		}, nil
	}
	next := a.Proposals[0]
...
func actorForAction(actionName string) actor.Actor {
	switch actionName {
	case ActionExecuteRestart, ActionIsolateInstance:
		return CloudAutomationActor
	default:
		return OpsAgentActor
	}
}
