...
func (a *ScriptedAgent) Propose(context.Context, AgentRequest) (AgentProposal, error) {
	if len(a.Proposals) == 0 {
		return AgentProposal{
			...
			ReasoningSummary: "scripted prior-auth agent has no remaining proposals",
			Confidence:       1,
		}, nil
	}
	next := a.Proposals[0]
...
func RecordAgentProposal(ctx context.Context, rt *runtime.Runtime, requestID, operatorInput string, proposal AgentProposal) error {
	actionName := proposal.ActionName
	if actionName == "NO_ACTION" {
		actionName = ""
	}
	return rt.ProposeDecision(ctx, decision.Decision{
		ID:             fmt.Sprintf("dec-%s-%d", requestID, time.Now().UnixNano()),
		EntityID:       requestID,
		...
		ActorID:          PriorAuthAgentActor.ID,
		CreatedAt:        time.Now().UTC(),
	})
}

func ApplyAgentProposal(ctx context.Context, rt *runtime.Runtime, requestID, currentState string, proposal AgentProposal) (action.ActionResult, error) {
	contract, err := Contract(rt, proposal.ActionName)
	if err != nil {
		return action.ActionResult{}, err
	}
	return rt.ExecuteAction(ctx, action.ActionContext{
		ActionName:   proposal.ActionName,
		EntityID:     requestID,
		EntityType:   EntityPriorAuthRequest,
...
func actorForAction(actionName string) actor.Actor {
	switch actionName {
	case ActionSubmitAuthorization, ActionSubmitReviewedAuthorization:
		return PayerPortalActor
	default:
		return PriorAuthAgentActor
	}
}
