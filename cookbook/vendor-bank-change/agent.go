...
func (a *ScriptedAgent) Propose(context.Context, AgentRequest) (AgentProposal, error) {
	if len(a.Proposals) == 0 {
		return AgentProposal{
			...
			ReasoningSummary: "scripted vendor bank-change agent has no remaining proposals",
			Confidence:       1,
		}, nil
	}
	next := a.Proposals[0]
...
func RecordAgentProposal(ctx context.Context, rt *runtime.Runtime, changeID, operatorInput string, proposal AgentProposal) error {
	actionName := proposal.ActionName
	if actionName == "NO_ACTION" {
		actionName = ""
	}
	return rt.ProposeDecision(ctx, decision.Decision{
		ID:             fmt.Sprintf("dec-%s-%d", changeID, time.Now().UnixNano()),
		EntityID:       changeID,
		...
		ActorID:          VendorAgentActor.ID,
		CreatedAt:        time.Now().UTC(),
	})
}

func ApplyAgentProposal(ctx context.Context, rt *runtime.Runtime, changeID, currentState string, proposal AgentProposal) (action.ActionResult, error) {
...
func actorForAction(actionName string) actor.Actor {
	switch actionName {
	case ActionApplyKnownAccount, ActionApplyApprovedChange:
		return VendorMasterActor
	default:
		return VendorAgentActor
	}
}
